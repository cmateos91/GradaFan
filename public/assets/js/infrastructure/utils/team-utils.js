...
const TEAM_BADGE_MAP = {
    ...
    'Atletico Madrid': '/assets/img/spain_atletico-madrid.svg',
    ...
    'Betis': '/assets/img/spain_real-betis.svg',
    ...
    'Athletic': '/assets/img/spain_athletic-club.svg',
    'Athletic Bilbao': '/assets/img/spain_athletic-club.svg',
    ...
    'RCD Espanyol': '/assets/img/spain_espanyol.svg',
    ...
    'Alaves': '/assets/img/spain_deportivo.svg',

    // Elche (ASCENDIDO 2025-26)
    'Elche CF': '/assets/img/spain_elche.svg',
    'Elche': '/assets/img/spain_elche.svg',

    // Levante (ASCENDIDO 2025-26)
    'Levante UD': '/assets/img/spain_levante.svg',
    'Levante': '/assets/img/spain_levante.svg',

    // Real Oviedo (ASCENDIDO 2025-26)
    'Real Oviedo': '/assets/img/spain_oviedo.svg',
    'Oviedo': '/assets/img/spain_oviedo.svg'
};
...
function getTeamBadge(teamName) {
    ...
    let badge = TEAM_BADGE_MAP[teamName];
    ...
    if (!badge) {
        ...
        for (const [key, value] of Object.entries(TEAM_BADGE_MAP)) {
            const keyLower = key.toLowerCase();
            if (keyLower.includes(teamNameLower) || teamNameLower.includes(keyLower)) {
                badge = value;
                console.log(`✅ Match parcial: "${teamName}" → "${key}"`);
                break;
            }
        }
    }
    ...
    if (!badge) {
        console.warn(`⚠️ getTeamBadge: badge no encontrado para "${teamName}" - usando logo LaLiga`);
